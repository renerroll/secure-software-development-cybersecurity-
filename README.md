# Secure Architecture Lab — Juice Shop analysis & рішення

## Architecture Lab — Juice Shop
Цей документ містить архітектурне ревʼю  локального контейнера **OWASP Juice Shop** (завдання виконано згідно `task.md`): знахідки, порушені принципи, рекомендації та короткі приклади виправлень.

---

## 🔍 Контекст запуску
- Образ: `bkimminich/juice-shop:latest` (має digest)
- Слухає порт: `3000`
- OpenAPI (Swagger) доступний на `/api-docs` (корисно для DAST)
- Package metadata та built server з контейнера були проаналізовані (`package.json`, `build/server.js`).

---

## 🔴 Щонайменше 5 архітектурних проблем (аналіз)

1) Хардкод secret у коді
- Файл: `build/server.js` (cookieParser('kekse'))
- Суть: секрети у коді → витік, відсутність обліку доступу
- Порушений принцип: PoLP, секрет-менеджмент
- Виправлення: використовувати Vault/SecretManager + CSI / `valueFrom.secretKeyRef`.

2) Публічний доступ до директорій і логів
- Файли/ендпоїнти: `/ftp`, `/support/logs` (serve-index)
- Суть: directory listing та логи можуть витікати у публічний доступ
- Порушений принцип: контрольованість, конфіденційність
- Виправлення: прибрати serve-index у проді; захистити доступ через RBAC/авторизацію.

3) Надмірно відкритий CORS / відключений XSS‑фільтр
- Файл: `build/server.js` (app.use(cors()), helmet.xssFilter закоментований)
- Суть: підвищена поверхня атаки (XSS, CSRF ризики)
- Виправлення: whitelist origin, увімкнути CSP/helmet XSS‑фільтр.

4) Використання старих/вразливих залежностей
- Файл: `package.json` (`jsonwebtoken@0.4.0`, `js-yaml@3.x` тощо)
- Суть: залежності з відомими CVE → supply-chain ризик
- Виправлення: SBOM + SCA (syft + trivy/grype), оновити/пінити пакунки.

5) Swagger доступний публічно (сигнал для DAST — добре, але в проді має бути обмеженим)
- Endpoint: `/api-docs`
- Суть: відкритий OpenAPI → DAST може сканувати, але у проді доступ слід контролювати
- Виправлення: обмежити доступ до swagger (auth / internal-only).

---

## ✅ Чеклист сигналів (поточний стан)
| Сигнал / Контроль               | Є / Немає | Коментар |
|---------------------------------|-----------|----------|
| OpenAPI / `response_model`      | ✅        | `/api-docs` доступний (позитив для DAST) |
| SBOM / package script           | ✅        | `npm run sbom` присутній |
| CI/CD pipeline                  | ✅/⚠️     | CI в репо (badges), але перевірте pipeline конфіг | 
| Secrets via Vault / CSI         | ❌        | секрети — у коді / env відсутні |
| Image pinning (digest)          | ✅        | образ має repoDigest |
| SCA (Trivy / Grype)             | ❌        | локально треба додати у CI |

---

## 🔧 Пріоритетні дії (план виправлень)
1. Міграція секретів у Vault CSI / Kubernetes Secret (критично).  
2. Закрити або захистити `/ftp`, `/support/logs` (високий пріоритет).  
3. Запустити SBOM + SCA у CI (syft -> sbom, trivy/grype, залежності pin/update).  
4. Обмежити CORS, увімкнути helmet.xssFilter/CSP (середній пріоритет).  
5. Обмежити доступ до `/api-docs` у продакшн (дозволити лише внутр. мережі або auth).

---

## Приклади змін / фрагменти
- Заміна хардкод‑секрету:
```js
// before
app.use(cookieParser('kekse'))

// after
app.use(cookieParser(process.env.COOKIE_SECRET))
// COOKIE_SECRET має інжектитись через Vault CSI / SecretKeyRef
```

- Гайд для CI (обов'язкові job-и): `sbom` → `trivy` → `semgrep` → `gitleaks`.

---

## 🔒 Vulnerabilities (to verify with SBOM/SCA)
Нижче — CVE, які були виявлені / надані. **Status** показує, чи вразливість підтверджена за `package.json` / runtime або потребує додаткової перевірки через SBOM/SCA (Trivy/Grype).

| CVE | Серйозність | Пакет | Версія | Статус | Рекомендації |
|---|---:|---|---:|---|---|
| CVE-2023-37903 | 9.8 | vm2 | 3.9.17 | Needs SBOM (likely transitive) | If present → upgrade/remove vm2; avoid running untrusted code in vm2 |
| CVE-2023-32314 | 9.8 | vm2 | 3.9.17 | Needs SBOM (likely transitive) | Patch or remove dependency |
| CVE-2026-22709 | 9.8 | vm2 | 3.9.17 | Needs SBOM (likely transitive) | Patch or remove dependency |
| CVE-2023-37466 | 9.8 | vm2 | 3.9.17 | Needs SBOM (likely transitive) | Patch or remove dependency |
| CVE-2021-44906 | 9.8 | minimist | 0.2.4 | Needs SBOM (transitive) | Update minimist or remove transitive dep |
| CVE-2025-55130 | 9.1 | node (runtime) | 22.21.1 | Confirmed (container runtime) | Upgrade Node to patched release or apply mitigation in image |
| CVE-2019-10744 | 9.1 | lodash | 2.4.2 | Needs SBOM (transitive) | Update lodash to safe version |
| CVE-2023-46233 | 9.1 | crypto-js | 3.3.0 | Needs SBOM (transitive) | Replace / update crypto-js |
| CVE-2015-9235 | N/A | jsonwebtoken | 0.1.0+ | Confirmed (package.json contains jsonwebtoken@0.4.0) | Upgrade `jsonwebtoken` to a patched version or migrate to `jose` |
| GHSA-5mrr-rgp6-x4gr | N/A | marsdb | 0.6.11 | Confirmed (top-level dep) | Check advisory; update or mitigate |

> Примітка: багато пакетів можуть бути транзитивними — потрібен SBOM (syft / npm run sbom) та SCA (trivy/grype) щоб підтвердити наявність і точні версії.

### How to verify locally
```bash
# generate SBOM for image (syft must be installed)
syft bkimminich/juice-shop:latest -o cyclonedx-json > sbom-image.json

# scan image
trivy image --format json --output trivy-image.json bkimminich/juice-shop:latest
# or
grype bkimminich/juice-shop:latest -o json > grype-image.json

# scan source
npm run sbom
npm audit
```

## Інструменти

| Інструмент | Призначення | Примітки |
|---|---|---|
| Postman | Надсилання запитів до API | Ручне тестування / сценарії |
| jwt.io | Перевірка / декодування JWT | Не зберігати секрети в сторонніх сервісах |
| Браузер | Перегляд веб‑інтерфейсу | Функціональна перевірка, DAST‑огляд |
| Notepad / Word / Google Docs | Оформлення звіту | Підготовка deliverables для LMS / репо |

---
---

