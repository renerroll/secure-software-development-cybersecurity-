# Підсумок Lab7 — SCA (Software Composition Analysis) та відповіді на `task.md`

Коротко: у цьому файлі зібрані відповіді та підсумки за `lab7.md` (аналіз npm‑залежностей із SCA‑інструментами) та пов’язане аналітичне завдання з `task.md` (SAST/DAST/IAST/RASP). Документ призначений як готовий підсумок для завантаження в LMS.
...
## 3) Приклад (зразок заповнення шаблону для `jsonwebtoken@8.5.1` — з `lab7.md`)
...
## 4) Відповіді на пункти з `task.md` (SAST / DAST / IAST / RASP)
- Де в SSDLC розташувати SCA (завдання lab7):  
...
- Приклад сценарію (RASP) для lab7:  
...
## 5) Рекомендації (після виконання Lab7)
...
## 7) Команди (коротко) — швидкий чек‑лист виконання локально
...
Файли/артефакти, які можу підготувати зараз:
- `README_lab7.md` (цей файл) ✔
- `sbom.json` (згенерувати за запитом)
- `osv_scan.json` / `grype_report.json` (згенерувати за запитом)

Хочеш, щоб я згенерував SBOM/Grype‑звіт зараз для одного з пакетів (наприклад `lodash@4.17.19`)? Якщо так — скажи, який пакет і я запущу команди й додам результати у `README_lab7.md`.  
